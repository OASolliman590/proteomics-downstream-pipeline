# Implementation Plan: Canonical protein input and biological identity

**Date:** 2026-09-12. **Spec:** [spec.md](spec.md). **Roadmap:** R02.
...
Prerequisites: R01 (`specs/002-foundation/`). Before dispatch, Maintainer verifies prerequisite commits and updates the packet starter with the actual helper names, schema versions and gate commands now present. Later packets remain outside this work. Canonical data/result/status fields are shared and cannot be redefined locally.
...
- `src/proteomics_pipeline/intake/canonical.py` — Canonical wide and long intake.
- `src/proteomics_pipeline/intake/scale.py` — Explicit scale and missing encoding.
- `src/proteomics_pipeline/intake/samples.py` — Biological observation hierarchy.
- `r/proteomicsCore/R/intake.R; tests/scientific/test_technical_units.py` — Explicit technical aggregation.
- `src/proteomics_pipeline/intake/features.py` — Feature annotations and protein groups.
- `src/proteomics_pipeline/intake/vendor.py; configs/mappings/` — Versioned vendor mappings.
- `src/proteomics_pipeline/intake/legacy.py` — Historical workbook importer.
- `src/proteomics_pipeline/intake/bundle.py` — Immutable canonical bundle.
- `src/proteomics_pipeline/intake/validation.py` — Intake validation report.
- `tests/contract/test_intake.py; tests/fixtures/synthetic/` — Input edge-case fixtures.
