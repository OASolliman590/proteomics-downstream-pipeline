# Implementation Plan: Archived-study regression, documentation and versioned successor

**Date:** 2026-09-12. **Spec:** [spec.md](spec.md). **Roadmap:** R12.
...
Prerequisites: R11 (`specs/012-validation/`). Before dispatch, Maintainer verifies prerequisite commits and updates the packet starter with the actual helper names, schema versions and gate commands now present. Later packets remain outside this work. Canonical data/result/status fields are shared and cannot be redefined locally.
...
- `scripts/maintained/legacy_regression.py` — Private local legacy adapter regression.
- `tests/regression/; docs/validation/legacy-comparison.md` — Legacy core numerical comparison.
- `docs/validation/legacy-comparison.md` — Scientific-change reconciliation.
- `configs/local example template; scripts/maintained/legacy_regression.py` — Full maintained study run.
- `docs/user-guide/; docs/methods/; README.md` — Comprehensive usage and methods docs.
- `specs/001-downstream-proteomics/traceability.json; progress.md` — Spec-task-evidence reconciliation.
- `scripts/maintained/build_release.py; .gitignore` — Source release hygiene.
- `scripts/maintained/build_release.py; docs/validation/delivery.json` — Versioned successor export.
- `docs/validation/final-acceptance.md` — Final integrated acceptance run.
- `CHANGELOG.md; specs/001-downstream-proteomics/progress.md` — Review-and-handoff completion.
