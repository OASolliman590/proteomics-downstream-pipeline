# Implementation plan — R03

**Phase:** 1. **Status:** frozen; dispatch requires accepted prerequisites and explicit authorization.
...
For V027, consume R01's canonical JSON hash utility and test the R03-resolved exclusion fragment inside a complete synthetic plan envelope; do not claim the R03 fragment is a real AnalysisPlan. Sol reruns V027 with V039 after R04 creates the actual plan. For V029, implement the narrow detection-only Fisher/BH endpoint entirely in the R03-owned `detection.R`; do not depend on R05. Sol reruns V029 after R05 to verify its family remains separate. These later regressions are Phase 1 barriers, not reasons to leave R03's own acceptance unexecutable.
