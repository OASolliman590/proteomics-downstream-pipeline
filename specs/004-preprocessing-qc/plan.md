# Implementation Plan: Preprocessing, missingness and quality control

**Date:** 2026-09-12. **Spec:** [spec.md](spec.md). **Roadmap:** R03.
...
Prerequisites: R02 (`specs/003-intake/`). Before dispatch, Maintainer verifies prerequisite commits and updates the packet starter with the actual helper names, schema versions and gate commands now present. Later packets remain outside this work. Canonical data/result/status fields are shared and cannot be redefined locally.
...
- `r/proteomicsCore/R/preprocess.R` — Preserve processed abundance by default.
- `r/proteomicsCore/R/normalization.R` — LFQ normalization policies.
- `r/proteomicsCore/R/tmt.R` — TMT plex and bridge policies.
- `r/proteomicsCore/R/filtering.R` — Contrast-aware coverage masks.
- `r/proteomicsCore/R/missingness.R` — Missingness mechanism diagnostics.
- `r/proteomicsCore/R/qc.R` — PCA and correlation diagnostics.
- `src/proteomics_pipeline/planning.py; r/proteomicsCore/R/qc.R` — Prespecified exclusions and watchlists.
- `r/proteomicsCore/R/sensitivity.R` — Explicit imputation sensitivities.
- `r/proteomicsCore/R/detection.R` — Detection-only exploratory endpoint.
- `src/proteomics_pipeline/reporting/qc_data.py; tests/integration/test_qc.py` — QC source data and stage report.
